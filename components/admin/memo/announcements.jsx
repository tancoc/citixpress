...
const AddModal = () => {
	...
	const addAnnouncement = useMutation((data) => api.create('/announcements', data), {
		onSuccess: () => {
	...
	return (
		<Modal title="Add Announcement" size="xl" toggle={(onOpen) => <IconButton size="lg" colorScheme="brand" icon={<FiPlus size={16} />} onClick={() => clearErrors() || reset() || onOpen()} />} disclosure={disclosure}>
...
const EditModal = ({ announcement }) => {
	...
	const editAnnouncement = useMutation((data) => api.update('/announcements', announcement._id, data), {
		onSuccess: () => {
			queryClient.invalidateQueries('announcements')
			setIsLoading(false)
			disclosure.onClose()
		...
		>
			<form onSubmit={handleSubmit(onSubmit)}>
				<Flex direction="column" gap={6}>
					<FormControl isInvalid={errors.description}>
						<FormLabel>Description</FormLabel>
						<Input defaultValue={announcement.description} size="lg" {...register('description', { required: true })} />
						<FormErrorMessage>This field is required.</FormErrorMessage>
					</FormControl>

					<FormControl isInvalid={errors.date}>
						<FormLabel>Date</FormLabel>
						<Input type="date" defaultValue={announcement.date} size="lg" {...register('date', { required: true })} />
						<FormErrorMessage>This field is required.</FormErrorMessage>
					</FormControl>
...
const Announcements = () => {
	const queryClient = useQueryClient()
	const { data: announcements, isFetched: isAnnouncementsFetched } = useQuery(['announcements'], () => api.all('/announcements'))
	const toast = useToast()

	const deleteAnnouncement = useMutation((data) => api.remove('/announcements', data), {
		onSuccess: () => {
			queryClient.invalidateQueries('announcements')
				...
				<Table
					...
					td={(announcement) => (
						<Tr key={announcement._id}>
							<Td>
								<Text textTransform="capitalize">{announcement.description}</Text>
							</Td>

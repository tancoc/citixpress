			...
			<Table
				...
				td={(user) => (
					<Tr key={user._id}>
						...
						<Td textAlign="right">
							<Button variant="tinted" colorScheme="brand" onClick={() => setSelectedUser(user) || disclosure.onClose()}>
								Select
							</Button>
						</Td>
					</Tr>
...
const AddModal = ({ users, isUsersFetched, departments, isDepartmentsFetched, designations, isDesignationsFetched }) => {
	...
	const addEmployee = useMutation((data) => api.update('/users/promote', selectedUser._id, data), {
		onSuccess: () => {
	...
	return (
		<Modal
			...
			size="xl"
			toggle={(onOpen) => (
				<Button size="lg" colorScheme="brand" onClick={() => setSelectedUser(null) || clearErrors() || reset() || onOpen()}>
					Add New
				</Button>
			)}
			disclosure={disclosure}
				...
				<Flex direction="column" gap={6}>
					<Flex justify="space-between" align="center" gap={6}>
						...
						{selectedUser ? (
							...
							<UserModal users={users} isUsersFetched={isUsersFetched} setSelectedUser={setSelectedUser} />
						)}
					</Flex>
...
const Employees = () => {
	...
	const { data: designations, isFetched: isDesignationsFetched } = useQuery(['designations'], () => api.all('/designations'))
					...
					<Table
						...
						td={(user) => (
							<Tr key={user._id}>
								...
								<Td>
									<Text textTransform="capitalize">{user.department}</Text>
								</Td>

								<Td>
									<Text textTransform="capitalize">{user.designation}</Text>
								</Td>

								<Td>
									<Badge variant="tinted" textTransform="capitalize" colorScheme={user.status === 'active' ? 'brand' : user.status === 'warning' ? 'yellow' : user.status === 'suspended' ? 'yellow' : user.status === 'restricted' ? 'red' : user.status === 'terminated' ? 'red' : user.status === 'resigned' && 'default'}>
										{user.status}
									</Badge>
								</Td>
						...
						select={(register) => (
							<Flex flex={1} justify="end" align="center" direction={{ base: 'column', md: 'row' }} gap={3}>
								...
								<Select placeholder="Designation" size="lg" w={{ base: 'full', md: 'auto' }} {...register('designation')}>
									{isDesignationsFetched &&
										designations.map((designation) => (
											<chakra.option textTransform="capitalize" value={designation.name} key={designation._id}>
												{designation.name}
											</chakra.option>
										))}
								</Select>

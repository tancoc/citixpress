...
import { Button, chakra, Divider, Flex, FormControl, FormErrorMessage, FormLabel, Input, Select, Text, useToast } from '@chakra-ui/react'
...
const Information = ({ user, departments, designations }) => {
	...
	const userInformation = useMutation((data) => api.update('/users', user._id, data), {
		onSuccess: () => {
			...
			<Card>
				<Flex direction="column" gap={6}>
					...
					<FormControl>
						...
						<Input value={user._id.toUpperCase()} size="lg" cursor="not-allowed" readOnly />
					</FormControl>
					...
					<Flex align="start" gap={6}>
						<FormControl>
							...
							<Select defaultValue={user.department} size="lg" {...register('department')}>
						...
						<FormControl>
							...
							<Select defaultValue={user.designation} size="lg" {...register('designation')}>
					...
					<Flex align="start" gap={6}>
						...
						<FormControl isInvalid={errors.contract_end_date}>
							...
							<Input type="date" defaultValue={user.contract_end_date} size="lg" {...register('contract_end_date')} />
						</FormControl>
					</Flex>
				</Flex>
			</Card>

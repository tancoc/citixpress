...
const AddModal = () => {
	...
	const addDesignation = useMutation((data) => api.create('/designations', data), {
		onSuccess: () => {
	...
	return (
		<Modal title="Add Designation" size="xl" toggle={(onOpen) => <IconButton size="lg" colorScheme="brand" icon={<FiPlus size={16} />} onClick={() => clearErrors() || reset() || onOpen()} />} disclosure={disclosure}>
...
const EditModal = ({ designation }) => {
	...
	const editDesignation = useMutation((data) => api.update('/designations', designation._id, data), {
		onSuccess: () => {
			queryClient.invalidateQueries('designations')
			setIsLoading(false)
			disclosure.onClose()
...
const Designations = () => {
	...
	const deleteDesignation = useMutation((data) => api.remove('/designations', data), {
		onSuccess: () => {
			queryClient.invalidateQueries('designations')

...
const AddModal = () => {
	...
	const addDepartment = useMutation((data) => api.create('/departments', data), {
		onSuccess: () => {
	...
	return (
		<Modal title="Add Department" size="xl" toggle={(onOpen) => <IconButton size="lg" colorScheme="brand" icon={<FiPlus size={16} />} onClick={() => clearErrors() || reset() || onOpen()} />} disclosure={disclosure}>
...
const EditModal = ({ department }) => {
	...
	const editDepartment = useMutation((data) => api.update('/departments', department._id, data), {
		onSuccess: () => {
			queryClient.invalidateQueries('departments')
			setIsLoading(false)
			disclosure.onClose()
...
const Departments = () => {
	...
	const deleteDepartment = useMutation((data) => api.remove('/departments', data), {
		onSuccess: () => {
			queryClient.invalidateQueries('departments')

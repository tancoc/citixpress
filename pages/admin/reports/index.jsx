...
import { FiDownloadCloud, FiFolder, FiMoreHorizontal, FiPlus, FiTrash2 } from 'react-icons/fi'
...
const AddReportTypeModal = () => {
	...
	const { data: types, isFetched: isTypesFetched } = useQuery(['report-types'], () => api.all('/reports/types'))
	...
	const addReportType = useMutation((data) => api.create('/reports/types', data), {
		onSuccess: () => {
			...
			toast({
				position: 'top',
				duration: 1000,
				render: () => <Toast title="Success" description="Report type successfully added." />
			})
		}
	})

	const deleteReportType = useMutation((data) => api.remove('/reports/types', data), {
		onSuccess: () => {
			queryClient.invalidateQueries('report-types')
			setIsLoading(false)
			...
			<Flex direction="column" gap={6}>
				<Flex direction="column" gap={6} position="relative" border="1px solid" borderColor="border" borderRadius={12} p={6}>
					<Flex justify="space-between" align="center" gap={6}>
						...
						<Badge variant="tinted" textTransform="capitalize" colorScheme={report.status === 'read' ? 'brand' : 'red'}>
							{report.status}
						</Badge>
					</Flex>
					...
					<Flex align="center" gap={3}>
						<Flex flex={1} align="center" gap={3}>
							...
							<Flex direction="column" w="calc(100% - 88px)">
								...
								<Text fontSize="xs">
									{report.file.size.toLocaleString(undefined, { maximumFractionDigits: 2 }) + ' '}
									{report.file.size >= 1 && report.file.size <= 999 && 'B'}
									{report.file.size >= 1000 && report.file.size <= 999999 && 'KB'}
									{report.file.size >= 1000000 && report.file.size <= 999999999 && 'MB'}
								</Text>
							</Flex>
						</Flex>
						...
						<Flex position="absolute" right={6}>
							<chakra.a href={report.file.url}>
								<IconButton size="xs" icon={<FiDownloadCloud size={16} />} isLoading={isLoading} onClick={onSubmit} />
							</chakra.a>
						</Flex>
					</Flex>
...
const Reports = () => {
	...
	const { data: reports, isFetched: isReportsFetched } = useQuery(['reports'], () => api.all('/reports'))
			...
			<Card>
				<Flex direction="column" gap={6}>
					...
					<Table
						...
						th={['Employee', 'Report Type', 'Description', 'Status', 'Created', '']}
						td={(report) => (
							<Tr key={report._id}>
								<Td>
									{users
								...
								<Td textAlign="right">
									<ViewModal session={session} users={users} report={report} />
								</Td>
							</Tr>
						)}
						select={(register) => (

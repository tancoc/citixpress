...
const AppLayout = (props) => {
	...
	const isUser = session ? (session.user.role === 'User' ? true : false) : true
	...
	if (status === 'loading') {
		...
		if ((!isEmployee && router.pathname.includes('user')) || (!isAdmin && router.pathname.includes('admin'))) {
			router.push('/')
			return null
		}
		...
		return (
			<chakra.div position="relative" h="auto" w="full">
				...
				<chakra.div position="relative" mx="auto" w="full" maxW={1536}>
					{!isUser && <Sidebar session={session} isAdmin={isAdmin} isEmployee={isEmployee} isUser={isUser} isSidebarOpen={isSidebarOpen} onSidebarClose={onSidebarClose} />}
					<chakra.main ml={{ base: 'full', lg: !isUser ? 256 : 'full' }} w="full" maxW={{ base: 'full', lg: !isUser ? 'calc(100% - 256px)' : 'full' }} {...props} />
				</chakra.div>
			</chakra.div>
		)
	}
